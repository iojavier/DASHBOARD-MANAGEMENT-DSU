...
def load_data(uploaded_file):
    ...
    df['DATE'] = df['DATE'].apply(lambda x: x.to_pydatetime() if isinstance(x, pd.Timestamp) else x)
                    ...
                    for col_num, col_name in enumerate(df_for_excel.columns):
                        ...
                        if col_name == 'DATE':
                            ...
                            if pd.isna(value) or value is None:
                                worksheet.write(current_row + row_num, col_num, '', formats['date'])
                            elif isinstance(value, (int, float)):  # Handle Unix timestamps
                                try:
                                    value = pd.Timestamp(value, unit='ms').to_pydatetime()
                                    worksheet.write_datetime(current_row + row_num, col_num, value, formats['date'])
                                except Exception:
                                    worksheet.write(current_row + row_num, col_num, str(value), formats['center'])
                                ...
                                worksheet.write_datetime(current_row + row_num, col_num, value.to_pydatetime(), formats['date'])
                            else:
                                worksheet.write(current_row + row_num, col_num, str(value), formats['center'])
...
def calculate_summary(df, remark_types, manual_correction=False):
    summary_columns = ['DATE', 'COLLECTORS', 'ACCOUNTS', 'TOTAL DIALED', 'PENETRATION RATE (%)', 
    ...
    for date, group in df_filtered.groupby(df_filtered['DATE'].dt.date):
        ...
        date_as_datetime = datetime.datetime.combine(date, datetime.time(0, 0))

        summary_table = pd.concat([summary_table, pd.DataFrame([{
            'DATE': date_as_datetime, 'COLLECTORS': collectors, 'ACCOUNTS': accounts, 'TOTAL DIALED': total_dialed,
...
def get_balance_summary(df, remark_types, manual_correction=False):
    ...
    for min_bal, max_bal, range_name in balance_ranges:
        ...
        if not balance_df.empty:
            ...
            result[f"Balance {range_name}"] = summary
    return result

# Define uploaded_files using Streamlit file uploader
